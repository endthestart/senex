...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table(u'custom_customproduct', (
            ('product', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['store.Product'], unique=True, primary_key=True)),
        ))
        db.send_create_signal(u'custom', ['CustomProduct'])

        # Adding M2M table for field option_group on 'CustomProduct'
        db.create_table(u'custom_customproduct_option_group', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('customproduct', models.ForeignKey(orm[u'custom.customproduct'], null=False)),
            ('optiongroup', models.ForeignKey(orm[u'store.optiongroup'], null=False))
        ))
        db.create_unique(u'custom_customproduct_option_group', ['customproduct_id', 'optiongroup_id'])
    ...
    def backwards(self, orm):
        ...
        db.delete_table('custom_customproduct_option_group')


    models = {
        u'custom.customproduct': {
            'Meta': {'object_name': 'CustomProduct'},
            'option_group': ('django.db.models.fields.related.ManyToManyField', [], {'to': u"orm['store.OptionGroup']", 'symmetrical': 'False', 'blank': 'True'}),
            'product': ('django.db.models.fields.related.OneToOneField', [], {'to': u"orm['store.Product']", 'unique': 'True', 'primary_key': 'True'})
        },
        u'store.category': {
